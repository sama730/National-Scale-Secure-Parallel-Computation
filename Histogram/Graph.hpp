...
public:
   ...
   // **************************  Apply **********************************************************************************************************************************
		...
		int numMasks = numUsers + sumNumItems;
		int numWires = numUsers + sumNumItems;
		...
		circuit = GradientDescendCircuitBuilder::BuildHistogramCircuit(elementsPerNode);
  ...
  //  // **************************  Scatter **********************************************************************************************************************************
   ...
   // **************************  Timing **********************************************************************************************************************************
		...
		cout << "phase" << " " << ShuffleDuration.count() << " " << GatherDuration.count() << " " << ApplyDuration.count() << endl;
	...
	void rerandomize(int party, Secret_Edge *secret_edges, int nEdges)
	{
		...
		std::vector<uint64_t> randUInt64 = rng->GetUInt64Array(3*nEdges);
		...
		if (party % 2 == 0)
		{
			...
			{
				secret_edges[edx].id_u += randUInt64[counter64]; counter64++;
				secret_edges[edx].id_v += randUInt64[counter64]; counter64++;
	...
	void SecretSharing(int party, int partner, Secret_Edge* secret_edges, int nEdges, Secret_Node* secret_nodes, int nUsers, int nItems, int alpha)
	{
		...
		std::vector<uint64_t> randUInt64 = rng->GetUInt64Array((nUsers + nItems) + 3*nEdges);
		...
		if ((party == Alice) || (party == Charlie))
		{
			...
			{
				secret_nodes[i].vertexID = randUInt64[counter64]; counter64++;
			}
			
			for (int i = 0; i < nEdges; i++) 
			{
				secret_edges[i].id_u = randUInt64[counter64]; counter64++;
				secret_edges[i].id_v = randUInt64[counter64]; counter64++;
		...
		else if ((party == Bob) || (party == David))
		{
			...
			for (int i = 0; i < (nUsers+nItems); i++)
			{
				secret_nodes[i].vertexID = (secret_nodes[i].vertexID -  randUInt64[counter64]); counter64++;
			}
			...
			for (int i = 0; i < nEdges; i++) 
			{
				secret_edges[i].id_u = (secret_edges[i].id_u - randUInt64[counter64]); counter64++;
				secret_edges[i].id_v = (secret_edges[i].id_v - randUInt64[counter64]); counter64++;
	...
	SecretEdgeMAC * computeMAC(int party, Secret_Edge* secret_edges, int nEdges)
	{
		...
		std::vector<uint64_t> randoms = rng->GetUInt64Array(3*nEdges);
	...
	void verifyMAC(int party, SecretEdgeMAC *MACedEdges, Secret_Edge *secretEdge, int nEdges)
	{
		...
		std::vector<uint64_t> myDifference(3*nEdges), theirDiffference(3*nEdges);
	...
	// 	{
	// 		cout << machineId << ": " << nodes[i].vertexID << " " << nodes[i].halfEdge.size() << endl;
	// 	}
	// }
	...
	// 	for (int i = 0; i < nEdges; i++)
	// 	{
	// 		cout << machineId << ": " << edges[i].id_u << " " << edges[i].id_v << " " << edges[i].isReal << endl;
	// 	}
	// }
};
